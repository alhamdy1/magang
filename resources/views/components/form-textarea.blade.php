{{-- 
Form Textarea Component - Accessible & Reusable
Usage: 
<x-form-textarea 
    name="keterangan" 
    label="Keterangan" 
...
<div class="mb-4">
    ...
    <textarea 
        ...
        {{ $attributes->merge([
            'class' => 'w-full px-4 py-2 border rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-y ' . 
    ...
    @if($maxlength)
        <p class="mt-1 text-xs text-gray-400 text-right">
            <span id="{{ $inputId }}-count">{{ strlen(old($name, $value)) }}</span>/{{ $maxlength }}
        </p>
        <script>
            document.getElementById('{{ $inputId }}').addEventListener('input', function() {
                document.getElementById('{{ $inputId }}-count').textContent = this.value.length;
            });
        </script>
    @endif
