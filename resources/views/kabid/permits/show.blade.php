...
@section('title', 'Review Final Permohonan - Kabid Penyelenggaraan')
...
<div class="mb-8 flex justify-between items-start">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Review Final Permohonan</h1>
        <p class="text-gray-600">{{ $permit->nama_pemohon }}</p>
    </div>
    <a href="{{ route('kabid.permits.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-300">
        Kembali
    </a>
</div>
...
<div class="mb-6 p-4 rounded-lg bg-green-100 border border-green-400">
    <p class="text-sm text-green-800">
        ...
        @if($permit->kasi_notes)
            <br><strong>Catatan Kasi:</strong> {{ $permit->kasi_notes }}
        @endif
    </p>
</div>
...
<!-- Action Buttons -->
<div class="mt-6 bg-white rounded-lg shadow p-6">
    <h2 class="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b">Keputusan Final</h2>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Approve Form -->
        <form action="{{ route('kabid.permits.approve', $permit) }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <button type="submit" class="w-full bg-green-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-700" onclick="return confirm('Apakah Anda yakin ingin menyetujui dan menyelesaikan permohonan ini? Nomor izin akan diterbitkan.')">
                ✓ Setujui & Terbitkan Izin
            </button>
        </form>
        
        <!-- Reject Form -->
        <form action="{{ route('kabid.permits.reject', $permit) }}" method="POST">
            @csrf
            <div class="mb-4">
